...
// Nominatim base-URL til udenlandske søgninger
 ...
 * (Foreløbig kun DK-adresser – udenlandske adresser håndteres via Nominatim
 * i hovedsøgefeltet og klik på kortet.)
 */
 ...
 * Først Dataforsyningen (DK). Hvis den ikke finder noget,
 * falder vi tilbage til Nominatim (global) med danske navne.
 ***************************************************/
map.on('click', function(e) {
  ...
  fetch(revUrl)
    .then(r => {
      if (!r.ok) {
        throw new Error("Reverse geocoding (DK) ikke OK");
      }
      return r.json();
    })
    .then(data => {
      // Hvis Dataforsyningen ikke returnerer noget brugbart, brug Nominatim som fallback
      if (!data || Object.keys(data).length === 0) {
        reverseGeocodeForeign(lat, lon);
        return;
      }
    ...
    .catch(err => {
      console.error("Reverse geocoding DK-fejl, prøver Nominatim:", err);
      reverseGeocodeForeign(lat, lon);
    });
});
...
async function updateInfoBox(data, lat, lon) {
  ...
  // KommuneOverlay kun når vi faktisk kender koderne
...
/***************************************************
 * Hjælper til Nominatim: byg "dansk" adresse-tekst
 ***************************************************/
function buildAddressFromNominatim(result) {
  ...
  const nameDa = namedetails["name:da"] || null;
  ...
  if (nameDa) {
    if (postcode || city) {
      displayName = `${nameDa}, ${postcode} ${city}`.trim();
    } else {
      displayName = nameDa;
    }
  } else if (road || housenumber || postcode || city) {
    displayName = `${road} ${housenumber}, ${postcode} ${city}`.trim();
  } else {
...
/***************************************************
 * Udenlandsk søgning (fallback) via Nominatim + JSONP
 ***************************************************/
function doForeignSearch(query, listElement) {
  const callbackName = "nominatimSearchCallback_" + Date.now();
  ...
  window[callbackName] = function(results) {
    ...
    results.forEach(result => {
      const info = buildAddressFromNominatim(result);
      const li = document.createElement("li");
      li.innerHTML = `🌍 ${info.displayName}`;
      li.addEventListener("click", function() {
        const lat = parseFloat(result.lat);
        const lon = parseFloat(result.lon);
        setCoordinateBox(lat, lon);
        placeMarkerAndZoom([lat, lon], info.displayName);

        const fakeData = {
          vejnavn: info.vejnavn,
          husnr: info.husnr,
          postnr: info.postnr,
          postnrnavn: info.postnrnavn,
          vejkode: "?",
          kommunekode: "?",
          adressebetegnelse: info.displayName
        };
        updateInfoBox(fakeData, lat, lon);
      });
      listElement.appendChild(li);
      searchItems.push(li);
    });
  ...
  script.src =
    ...
    `addressdetails=1&namedetails=1&accept-language=da&limit=10&` +
...
/***************************************************
 * Reverse geocoding uden for DK via Nominatim + JSONP
 ***************************************************/
function reverseGeocodeForeign(lat, lon) {
  const callbackName = "nominatimReverseCallback_" + Date.now();
  const script = document.createElement("script");
  ...
  window[callbackName] = function(result) {
    ...
    if (!result) {
      console.warn("Nominatim reverse gav intet resultat.");
      return;
    }
    ...
    updateInfoBox(fakeData, lat, lon);
    fillRouteFieldsFromClick(fakeData, lat, lon);
  };
  ...
  script.src =
    ...
    `addressdetails=1&namedetails=1&accept-language=da&` +
    `json_callback=${callbackName}`;
  script.referrerPolicy = "no-referrer-when-downgrade";
  document.body.appendChild(script);
}
 ...
 * Primært DK (Dataforsyningen). Hvis der INTET findes, prøver vi
 * Nominatim globalt med danske navne som fallback.
 ***************************************************/
function doSearch(query, listElement) {
  ...
  .then(([addrData, stedData, roadData, strandData]) => {
    ...
    // Hvis DK-søgningen ikke giver nogen resultater, prøv udenlandsk søgning via Nominatim
    if (combined.length === 0) {
      doForeignSearch(query, listElement);
      return;
    }
...
infoCloseBtn.addEventListener("click", function() {
  document.getElementById("infoBox").mongo_display = "none";
  document.getElementById("statsvejInfoBox").style.display = "none";
  if (currentMarker) {
    map.removeLayer(currentMarker);
    currentMarker = null;
